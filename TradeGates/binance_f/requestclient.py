...
class RequestClient(object):
    ...
    def post_order(self, symbol: 'str', side: 'OrderSide', ordertype: 'OrderType',
                   ...
                   activationPrice: 'float' = None, priceProtect: 'boolean' = None,
        ...
        """
        response = call_sync(
            self.request_impl.post_order(symbol, side, ordertype, timeInForce, quantity, reduceOnly, price,
                                         newClientOrderId, stopPrice,
                                         workingType, closePosition, positionSide, callbackRate, activationPrice,
                                         priceProtect, newOrderRespType))
        self.refresh_limits(response[1])
        return response[0]

    def post_batch_order(self, batchOrders: 'list') -> any:
        """
        New Orders Batch (TRADE)

        POST /fapi/v1/batchOrder (HMAC SHA256)

        Send in multiple new orders.
        """
        response = call_sync(
            self.request_impl.post_batch_order(batchOrders))
        self.refresh_limits(response[1])
        return response[0]
